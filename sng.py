...
def parse_arguments():
    ...
    parser.add_argument('-n', '--numnames', action='store', dest='numnames',
            type=int, default=15)
    parser.add_argument('-W', '--wordfile', action='store', dest='wordfile',
            default=None)
    parser.add_argument('-T', '--templatefile', action='store',
            dest='templatefile', default=None)
...
class SongNameGenerator(object):
    ...
    def __init__(self, wordlist=[], templates=[], wordfile=None,
                 templatefile=None):
    ...
    def generate_name(self):
        ...
        return template.format(self._get_random_word(),
...
def main(parsed):
    init()
    bg_colors = [Back.GREEN, Back.YELLOW, Back.RED]
    fg_colors = [Fore.BLACK, Fore.BLACK, Fore.YELLOW]
    ...
    for i in range(0, numnames):
        ...
        print bg_colors[i % len(bg_colors)], fg_colors[i % len(fg_colors)], \
              name.upper(), Back.RESET, Fore.RESET
    return
